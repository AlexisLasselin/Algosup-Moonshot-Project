...
while cap.isOpened():
    ...
    if pose_results.pose_landmarks:
        ...
        for key, points in keypoint_positions.items():
            for point in points:
                cv2.circle(
                    frame, point, 6, (255, 0, 0), cv2.FILLED
                )  # Blue circles for pose keypoints

        # Connect shoulders, elbows, and hips
        cv2.line(
            frame,
            keypoint_positions["shoulders"][0],
            keypoint_positions["shoulders"][1],
            (255, 0, 0),
            3,
        )
        cv2.line(
        ...
        cv2.line(
            frame,
            keypoint_positions["shoulders"][1],
            keypoint_positions["elbows"][1],
            (255, 0, 0),
            3,
        )
        cv2.line(
            frame,
            keypoint_positions["hips"][0],
            keypoint_positions["hips"][1],
            (255, 0, 0),
            3,
        )
        cv2.line(
